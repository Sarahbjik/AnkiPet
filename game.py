...
def getStats():
    ...
    # working
    cards, thetime, failed, lrn, rev, relrn, filt = col.db.first("""
...
from revlog where id > ? """+"", (col.sched.dayCutoff-86400)*1000)
    cards = cards or 0     # cards done today
    thetime = thetime or 0     # time spent today
    failed = failed or 0       
    lrn = lrn or 0
    rev = rev or 0
    relrn = relrn or 0
...
and due = ?""" % ids2str([d['id'] for d in col.decks.all()]), col.sched.today-day)
    
    # cards to review and learn per deck
    #entries of all decks with an example of:[['Collect', 1548530072925, 13=torev, 0=learn, 6=new], ['Gefilterter Stapel 1', 1558121480968, 0, 0, 0], ...]
    todoStats=col.sched.deckDueList()
        
        
    # now I need due today per deck or done today per deck
    # I dont count the cards that are re-learned
    lim="cid in (select id from cards where did in %s)" % ids2str(col.decks.active())
    lims = []
    lims.append("id > %d" % ((col.sched.dayCutoff-(20*1*86400))*1000))
    if lim:
        lims.append(lim)
    if lims:
